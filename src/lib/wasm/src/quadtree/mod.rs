...
impl QTPoint {
    ...
    #[inline]
    pub fn distance(&self, other: &QTPoint) -> f64 {
...
impl Item {
    ...
    #[inline]
    pub fn get_center(&self) -> QTPoint {
        match self {
...
impl Rect {
    ...
    #[inline]
    pub fn contains_point(&self, p: &QTPoint) -> bool {
...
fn construct_quadtree(items: Vec<(Rect, ItemId)>, total_bbox: Rect, max_len: usize) -> Vec<Knot> {
    // 100 - 3% of points
    let max_len = max_len.max(1).min(33);

    // max_len = percentage
    // max_len = 10 -> each knot can contain at maximum 10% of all points
    let max_len = ((items.len() as f32 / max_len as f32).ceil() as usize)
        .max(20)
        .min(items.len() + 1);
    ...
    while !items_to_search.is_empty() {
        ...
        for id in items_to_search {
            match &tree.knots[id.0] {
                ...
                Knot::HasChildren(
                    [(tl_id, tl_rect), (tr_id, tr_rect), (bl_id, bl_rect), (br_id, br_rect)],
                ) => {
                    if query(tl_rect, query_rect) {
                        knots_to_resolve.push(*tl_id);
                    }
                    if query(tr_rect, query_rect) {
                        knots_to_resolve.push(*tr_id);
                    }
                    if query(bl_rect, query_rect) {
                        knots_to_resolve.push(*bl_id);
                    }
                    if query(br_rect, query_rect) {
                        knots_to_resolve.push(*br_id);
                    }
                }
            }
        }
...
impl QuadTree {
    ...
    pub fn new_with_max_items_per_quad<I: Iterator<Item = (ItemId, Item)>>(
        items: I,
        max_items: usize,
    ) -> Self {
        let items_with_id_and_bbox = items
            .into_iter()
            .map(|(id, i)| (i, i.get_bbox(), id))
            .collect::<Vec<_>>();
        let all_items = items_with_id_and_bbox
            .iter()
            .map(|(item, _bbox, id)| (*id, *item))
            .collect::<BTreeMap<_, _>>();
        let all_bboxes = items_with_id_and_bbox
            .into_iter()
            .map(|(_item, bbox, id)| (bbox, id))
            .collect::<Vec<_>>();
    ...
    pub fn get_points_contained_by(&self, rect: &Rect) -> Vec<QTPoint> {
